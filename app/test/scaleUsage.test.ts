...
const RAW_TYPE_DEBT: Record<string, number> = {
  ...
  // 27 until the stepper moved to components/setup/StageStepper.tsx, then 26
  // until step 4 moved to components/setup/ReviewStage.tsx with its five.
  // Nothing was cleaned up in either move; the entries below carry the same
  // counts under their new paths.
  'app/setup/page.tsx': 21,
  ...
  'components/setup/ReviewStage.tsx': 5,
...
const OFF_SCALE_GAP_DEBT: Record<string, number> = {
  ...
  // 30 until the stepper moved out with its `gap-2` and `mt-1`, then 28 until
  // step 4 moved out with `mt-1`, `mt-4` and `mt-5`.
  'app/setup/page.tsx': 25,
  'components/AccountsPage.tsx': 3,
  'components/AgentAccessPanel.tsx': 3,
  ...
  'components/setup/ReviewStage.tsx': 3,
