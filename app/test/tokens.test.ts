...
/**
 * The favicon is a standalone file: no CSS custom property reaches it, so its
 * colours are a copy of the palette. Copies drift, and this one had -- after
 * §4 lifted the grounds it still painted #0B0D10 and #07090B, so the image a
 * reader sees before the page loads was the last surface in the old palette.
 *
 * §17. The OG image is a raster and cannot be checked this way; it is
 * generated by scripts/og.mjs, which reads globals.css instead of copying it.
 */
describe('the favicon is painted in this palette', () => {
  ...
  it('uses no colour the palette does not have', () => {
    ...
    // it governs is the one that gets forgotten again.
    const drawn = icon.replace(/<!--[\s\S]*?-->/g, '')
    const used = [...drawn.matchAll(/#[0-9A-Fa-f]{6}/g)].map((m) => m[0].toUpperCase())
