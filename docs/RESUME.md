...
| `cd app && pnpm run test` | 502/502, re-run 2026-09-19 (+5 in `surface.test.ts` for the landing's motion exception; 497 before it, 485 after the attribution work; +12 for describeTopUpForOwner, refuseNomination and refuseGrant. Was 480, not the 481 written here: `attribution.test.ts` adds 5, holding every owner write to the suffix and watching it reach a fake wallet. Before that 500 on 2026-09-17; the emitted block went from five variables to two, taking `displayTag`/`isAttributionTag` and their 19 tests with it — the bug they guarded cannot occur in a block that carries no tag) |
| `cd app && pnpm run test:e2e` | 53/53 local, re-run 2026-09-19 on the build Playwright starts itself (+4 in `motion.spec.ts`; +6 in `owner.spec.ts`, the owner header on a phone, and +2 lining the meter up with the panels); 41/41 on 2026-09-18 against <https://leash-app-phi.vercel.app> with `LEASH_E2E_URL`. The deployed run was 40/41 until the redeploy: `landing.spec.ts` asserts the landing links to the v2 account and the build then serving that URL still carried v1's. That failure was the deploy signal, and it cleared the moment the deploy landed. |
